...
function buildRiverNetwork({
    ...
    // ============================================================
    // 1. ПРОВЕРКА: SVG-элемент
    // ============================================================
    const svgElement = typeof svg === "string" ? document.querySelector(svg) : svg;
    if (!svgElement) {
        console.warn('❌ buildRiverNetwork: SVG элемент не найден:', svg);
        return;
    }
    ...
    if (!sourceEl) {
        console.warn('❌ buildRiverNetwork: источник не найден:', source);
        return;
    }

    // ============================================================
    // 3. ПРОВЕРКА: Цели (фильтруем null)
    // ============================================================
    ...
    if (!targetEls.length) {
        console.warn('❌ buildRiverNetwork: нет доступных целей для:', source);
        return;
    }

    console.log('✅ buildRiverNetwork: все элементы найдены, рисуем реку:', {
        source: source,
        targets: targets,
        className: className
    });
...
// ============================================================
// КОНТАКТЫ — ПЕРЕКЛЮЧЕНИЕ КАРТА / ФОТО (добавлено к существующему коду)
// ============================================================
// ============================================================
